...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if ex_index < 5:
    ...
    #tf.logging.info("image (first 10 elements): %s" % " ".join([str(x) for x in image))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["image"] = create_byteslist(feature.image)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, image,
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
  ...
  for feature in features:
    ...
    all_image.append(feature.image)
